...
class WeChatApp(App):
    def build(self):
        ...
        # 初始化完整历史记录列表（存储所有消息）
        self.full_history = []  # 每个元素: (is_user, text, timestamp)
        ...
        # 左侧标题
        title = Label(text="智能管家", color=(0.1, 0.1, 0.1, 1), bold=True, size_hint_x=0.7)
        header.add_widget(title)

        # 右侧功能按钮
        btn_layout = BoxLayout(size_hint_x=0.3, spacing=dp(5))
        history_btn = Button(text="历史", size_hint_x=0.5, background_color=(0.8,0.8,0.8,1), color=(0,0,0,1))
        history_btn.bind(on_press=self.show_history)
        export_btn = Button(text="导出", size_hint_x=0.5, background_color=(0.8,0.8,0.8,1), color=(0,0,0,1))
    ...
    def add_bubble(self, text, is_user):
        ...
        self.full_history.append((is_user, text, timestamp))
    ...
    def show_history(self, instance):
        if not self.full_history:
            self.add_bubble("当前没有对话记录。", is_user=False)
            return
        
        # 生成历史文本
        history_lines = ["======== 完整对话历史 ========"]
        for is_user, text, ts in self.full_history:
            speaker = "用户" if is_user else "AI"
            history_lines.append(f"[{ts}] {speaker}: {text}")
        history_text = "\n".join(history_lines)
        # 将历史记录以系统消息方式显示（用AI气泡）
        self.add_bubble(history_text, is_user=False)

    # ---------- 新增功能：导出对话 ----------
    def export_history(self, instance):
        if not self.full_history:
            self.add_bubble("当前没有对话记录可导出。", is_user=False)
            return
        ...
        try:
            ...
            self.add_bubble(f"✅ 对话已导出至：{filepath}", is_user=False)
        except Exception as e:
            self.add_bubble(f"❌ 导出失败：{str(e)}", is_user=False)
